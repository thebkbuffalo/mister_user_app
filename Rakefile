...
namespace :db do
  ...
  desc "this will seed 10 random users"
  task :seed_random_user, :env do |cmd, args|
    puts "grabbing some random people..."
    env = args[:env] || ENV["RACK_ENV"] || "development"
    Rake::Task['environment'].invoke(env)
    puts "loading the environment"
    puts "grabbing some random peoples"
    RandomUser.fetch
  end
end # end's namespace db
